...
const myPlaylist = async (req, res, next) => {
    try {
        const playlist = await PlayListModel.findOne({user: req.user.id}).populate("user")
        res.json(playlist)
    } catch (error) {
        next(error)
    }
...
module.exports = {register, login, me, myPlaylist};
